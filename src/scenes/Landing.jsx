...
const Landing = ({ setSelectedPage }) => {
  ...
  return (
    ...
    >
      {/* SVG ANIMATIONS - Responsive visibility */}
      <FloatingParticles isVisible={isAboveMedium} />
      <CodeBrackets isVisible={isAboveSmall} />
      <MorphingBlob isVisible={isAboveMedium} />
      <PulseWave isVisible={isAboveLarge} />
      <GeometricSpinner isVisible={isAboveMedium} />

      {/* MAIN CONTENT */}
      <div className="w-full max-w-7xl mx-auto flex flex-col items-center justify-center z-30">
        {/* NAME SECTION */}
        <div className="w-full text-center mb-8 sm:mb-12 md:mb-16">
          <div
            className={`
              ...
              font-playfair z-10 text-center 
              flex flex-wrap justify-center items-center 
              gap-1 w-full leading-tight
            `}
          >
            <span
              className="overflow-hidden relative z-1 cursor-none p-2"
              ref={ref1}
            >
              <AnimatedLetters
                text={"DEVANSH"}
        ...
        {/* DESCRIPTION SECTION */}
        <div className="w-full flex items-center justify-center relative z-5">
          <div
            className={`
              w-full max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl
              rounded-2xl backdrop-blur-3xl relative overflow-hidden
              ${isAboveLarge ? "cursor-none" : ""}
            `}
            ref={ref3}
          >
            {isAboveLarge && (
            ...
            <div className="backdrop-blur-[100px] min-w-full min-h-full p-4 sm:p-5 md:p-6 rounded-2xl border border-white/30 z-4">
              <h3
                className={`
                text-center bg-clip-text bg-white underline mb-3 sm:mb-4
                ${isAboveMedium ? "text-xl md:text-2xl" : "text-lg"}
              `}
              >
                Software Developer
              </h3>
        ...
        {!isAboveLarge && (
          <div className="mt-8 sm:mt-10 md:mt-12 w-full flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6">
            <AnchorLink
              className="w-full sm:w-auto bg-[linear-gradient(90deg,_#24CBFF_14.53%,_#FC59FF_69.36%,_#FFBD0C_117.73%)] 
                         text-[#010026] rounded-sm py-3 px-6 sm:py-3.5 sm:px-10.5 
                         font-medium text-center text-sm sm:text-base
...
// SVG Animation Components with responsive visibility
const FloatingParticles = ({ isVisible }) => {
  if (!isVisible) return null;

  return (
    <div className="absolute inset-0 w-50 h-50 top-80 left-[50vw] pointer-events-none">
      <svg
        className="absolute top-10 left-4 sm:left-10 opacity-60"
        width="40 sm:50"
        height="40 sm:50"
        viewBox="0 0 60 60"
      >
      ...
      <svg
        className="absolute top-20 sm:top-32 right-4 sm:right-20 opacity-50"
        width="30 sm:40"
        height="30 sm:40"
        viewBox="0 0 40 40"
      >
      ...
      <svg
        className="absolute top-1/3 right-4 sm:right-10 opacity-60"
        width="25 sm:35"
        height="25 sm:35"
        viewBox="0 0 35 35"
      >
...
const CodeBrackets = ({ isVisible }) => {
  if (!isVisible) return null;

  return (
    <div className="absolute bottom-4 sm:bottom-10 right-4 sm:right-10 pointer-events-none opacity-30">
      <svg width="60 sm:80" height="60 sm:80" viewBox="0 0 80 80">
        <path
          d="M20,20 L10,20 L10,40 L10,60 L20,60"
          fill="none"
          stroke="#24CBFF"
...
const MorphingBlob = ({ isVisible }) => {
  if (!isVisible) return null;

  return (
    <div className="absolute top-1/2 left-4 sm:left-10 pointer-events-none opacity-40">
      <svg width="80 sm:100" height="80 sm:100" viewBox="0 0 100 100">
        <path fill="#FC59FF" opacity="0.6">
          <animate
            attributeName="d"
            values="M50,10 Q80,40 50,70 Q20,40 50,10 Z;
...
const GeometricSpinner = ({ isVisible }) => {
  if (!isVisible) return null;

  return (
    <div className="absolute w-50 top-1/4 left-1/8 pointer-events-none opacity-10">
      <svg width="50 sm:60" height="50 sm:60" viewBox="0 0 60 60">
        <g style={{ transformOrigin: "30px 30px" }}>
          <polygon
            points="30,10 40,20 40,40 30,50 20,40 20,20"
            fill="none"
